...
const AdminViews = {
  ...
  renderDashboard: function() {
    if (Store.adminCache.loading.dashboard) {
      return `
        <div style="padding: 3rem; text-align: center; color: var(--text-muted);">
          ...
          <p>Connecting to MongoDB & Loading Administration Dashboard...</p>
        </div>
      `;
    }

    if (Store.adminCache.error.dashboard) {
      return this.renderErrorState(
        "Failed to Load Administration Dashboard",
        Store.adminCache.error.dashboard,
        "Store.syncAdminDashboard().then(() => App.renderCurrentView())"
      );
    }

    if (!Store.adminCache.dashboard) {
      Store.syncAdminDashboard().then(() => App.renderCurrentView());
      return `
        <div style="padding: 3rem; text-align: center; color: var(--text-muted);">
          <div class="spinner" style="margin: 0 auto 1rem auto; width: 36px; height: 36px; border: 3px solid var(--border-color); border-top-color: var(--accent-amber); border-radius: 50%; animation: spin 1s linear infinite;"></div>
    ...
    const dbData = Store.adminCache.dashboard || {};
    const stats = dbData.stats || Store.getAdminStats() || {};
    const upcomingEvents = dbData.upcomingEvents || [];
    const recentAnnouncements = dbData.recentAnnouncements || [];

    const totalStudents = stats.totalStudents || 1250;
    const totalStaff = stats.totalStaff || 84;
    const totalDepts = stats.totalDepartments || stats.departments || 6;
    const totalCourses = stats.totalCourses || 42;
    ...
    return `
      <div>
        <div class="card glass-panel" style="background: linear-gradient(135deg, rgba(217, 119, 6, 0.2) 0%, rgba(124, 58, 237, 0.1) 100%); margin-bottom: 2rem; border-color: rgba(245, 158, 11, 0.3);">
          <div style="display: flex; justify-content: space-between; align-items: center; flex-wrap: wrap; gap: 1rem;">
            <div>
              ...
              <p style="color: var(--text-muted); font-size: 0.95rem;">System status normal • ${totalStudents} enrolled students • ${totalDepts} active departments • Real-Time MongoDB Connected</p>
            </div>
        ...
        <div class="grid-cols-4" style="margin-bottom: 2rem;">
          ...
          <div class="stat-card">
            <div class="stat-icon" style="background: rgba(6, 182, 212, 0.15); color: #06b6d4;"><i data-lucide="book-open"></i></div>
            <div class="stat-info">
          ...
          <div class="card">
            ...
            <div style="display: flex; flex-direction: column; gap: 0.85rem;">
              ${upcomingEvents.length === 0 ? `
                <div style="padding: 1.5rem; text-align: center; color: var(--text-muted);">No upcoming events in database.</div>
              ` : upcomingEvents.map(e => `
                <div style="padding: 0.85rem; background: var(--bg-secondary); border-radius: var(--radius-sm); display: flex; justify-content: space-between; align-items: center;">
                  <div>
                    ...
                    <span style="font-size: 0.78rem; color: var(--text-muted); display: block;">${e.location || 'Campus Auditorium'} • ${e.date ? new Date(e.date).toLocaleDateString() : 'Upcoming'}</span>
                  </div>
                  <span class="badge badge-${e.status === 'Approved' ? 'staff' : 'admin'}">${e.status || 'Scheduled'}</span>
                </div>
              `).join('')}
            </div>
          </div>
          ...
          <div class="card">
            ...
            <div style="display: flex; flex-direction: column; gap: 0.85rem;">
              ${recentAnnouncements.length === 0 ? `
                <div style="padding: 1.5rem; text-align: center; color: var(--text-muted);">No announcements published yet.</div>
              ` : recentAnnouncements.map(a => `
                <div style="padding: 0.85rem; background: var(--bg-secondary); border-radius: var(--radius-sm); display: flex; justify-content: space-between; align-items: center;">
                  <div>
                    <strong style="font-size: 0.9rem;">${a.title}</strong>
                    <span style="font-size: 0.78rem; color: var(--text-muted); display: block;">Target: ${a.targetPortal || a.target || 'All'}</span>
                  </div>
                  <span class="badge badge-${a.priority === 'High' ? 'danger' : 'primary'}">${a.priority || 'Normal'}</span>
                </div>
              `).join('')}
            </div>
          </div>
