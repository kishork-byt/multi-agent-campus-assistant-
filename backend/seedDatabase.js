...
async function seedDatabase() {
  ...
  try {
    if (mongoose.connection.readyState === 1) {
      console.log("⚡ Using existing active MongoDB connection for seeding...");
      isAtlas = !mongoose.connection.host?.includes("127.0.0.1") && !mongoose.connection.host?.includes("localhost");
    } else {
